...
public class Spiker : Enemy
{
            ...
            for (int x = 0; x < neighbors.GetLength(0); x++)
            {
                for (int y = 0; y < neighbors.GetLength(1); y++)
                {
                    ...
                    if (tempGrid[x, y].occupied && tempGrid[x, y].occupant is Player &&
                        tempGrid[x, y].occupant == _currTarget)
                    {
                        playerInRange = true;
                        break;
                    }
    ...
    private DirToMove FindDirMoveIn(CloserIn closerIn)
    {
        ...
        if (closerIn == CloserIn.X) 
            return playerCoord.X > spikerCoord.X ? DirToMove.negX : DirToMove.posX;
        
        return playerCoord.Y > spikerCoord.Y ? DirToMove.negY : DirToMove.posY;
    }

    private DirToMove InverseDirToMove(DirToMove dirToMove)
    {
        if (dirToMove == DirToMove.negX) return DirToMove.posX;
        if (dirToMove == DirToMove.posX) return DirToMove.negX;
        if (dirToMove == DirToMove.negY) return DirToMove.posY;
        if (dirToMove == DirToMove.posY) return DirToMove.negY;
    ...
    private DirToMove InspectBestTurnDirection(DirToMove dirToMove)
    {
        ...
        if (dirToMove == DirToMove.negX || dirToMove == DirToMove.posX)
        {
            ...
            newDir = targetCoord.Y > spikerCoord.Y ? DirToMove.negY : DirToMove.posY;
        }
        else
        {
            //Means we were originally trying to move up or down.
            //Now We want to move left or right.
            newDir = targetCoord.X > spikerCoord.X ? DirToMove.negX : DirToMove.posX;
        }
